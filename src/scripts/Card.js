...
const Card = async (name, position, facebook, instagram, discord, ava) => {
	...
	context.fillText(name, canvas.width / 2, 240);
	...
	context.textAlign = 'center';
	context.fillText(position, canvas.width / 2, 270);
	...
	context.fillText(facebook, 75, 331);
	...
	context.fillStyle = '#C5C5DA';
	context.textAlign = 'left';
	context.fillText(instagram, 75, 407);
	...
	context.fillStyle = '#C5C5DA';
	context.textAlign = 'left';
	context.fillText(discord, 75, 485);
	 ...
	 */
	const avatar = await ImagesCanvas.loadImage(ava);
	context.beginPath();
